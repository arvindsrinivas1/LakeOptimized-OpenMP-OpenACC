...
/*
Single Author info:
asubram9 Arvind Srinivas Subramanian
*/
...
#define SMALL_JUNK_SIZE 1
#define MEDIUM_JUNK_SIZE 1000
#define LARGE_JUNK_SIZE 10000
...
void run_sim(double *u, double *u0, double *u1, double *pebbles, int n, double h, double end_time)
{
 ...
 /* V2 */
  ...
  int k = 0;
  #pragma omp parallel for shared(uo, u0) private(k) num_threads(nthreads)
  for(k = 0; k < n*n; k++){
    uo[k] = u0[k];
  }

  #pragma omp parallel for shared(uc, u1) private(k) num_threads(nthreads)
  for(k = 0; k < n*n; k++){
    uc[k] = u1[k];
  }
  ...
  /* Refer the method used in https://developer.nvidia.com/blog/openacc-example-part-1/ */

  /* This is basically because if we use the copyin class inside while, then for every iteration we copyin values 
  copyout after loop completion, which is unecessary work. Also, we replaced copy with copyin and copyout because we 
  should/ need not copy out other pointers(Arrays) as we only need the value in un*/
  #pragma acc data copyin(dt,end_time,uc[:n*n],uo[:n*n],pebbles[:n*n]) copyout(un[:n*n])
  while(1)
  {
    ...
    /* Ask the compiler to generate a GPU code. Using presenet to tell that the mem is already there inside is faster
    than  copying in again, which is just redundant since we did it already outside while
    https://developer.nvidia.com/blog/openacc-example-part-1/ */
    #pragma omp parallel for private(i,j,idx) shared(un, uc, uo) num_threads(nthreads)
    #pragma acc kernels loop present(dt,end_time, uc[:n*n],uo[:n*n],pebbles[:n*n], un[:n*n])
    for( i = 0; i < n; i++)
    {
      /*#pragma omp parallel for private(i,j,idx) shared(un, uc, uo) num_threads(nthreads) */
      for( j = 0; j < n; j++)
...
void init(double *u, double *pebbles, int n)
{
  ...
  #pragma omp parallel for shared(u) private(idx,i,j) num_threads(nthreads)
  for(i = 0; i < n ; i++)
  {
    for(j = 0; j < n ; j++)
    {
